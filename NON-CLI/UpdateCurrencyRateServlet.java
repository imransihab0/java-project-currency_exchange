...
public class UpdateCurrencyRateServlet extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        double newRateToBdt = Double.parseDouble(request.getParameter("rateToBdt"));

        try (Connection conn = DBConnection.getConnection()) {
            String query = "UPDATE exchange_rates SET rate_to_bdt = ? WHERE currency_code = ?";
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.setDouble(1, newRateToBdt);
            ...
            if (rowsAffected > 0) {
                ...
                renderForm(response, "Currency not found!");
            }
        } catch (SQLException e) {
            renderForm(response, "Error: " + e.getMessage());
        }
    }

    private void renderForm(HttpServletResponse response, String errorMsg) throws IOException {
        response.setContentType("text/html");
        PrintWriter out = response.getWriter();

        out.println("<html><head><title>Update Currency Rate</title>");
        out.println("<style>");
        ...
        out.println(".error { color: red; margin-top: 15px; font-weight: bold; }");
        out.println("</style>");
        out.println("</head><body>");

        out.println("<div class='container'>");
        out.println("<h1>Update Currency Rate</h1>");
        out.println("<form method='POST'>");
        out.println("Currency Code (e.g. USD): <input type='text' name='currencyCode' required>");
        out.println("New Rate to BDT: <input type='number' name='rateToBdt' step='any' required>");
